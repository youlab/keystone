...
# Load your CSV file (replace 'data.csv' with the path to your file)
...
bar_positions = [3.3, 4.235, 4.92, 0.0, 1.21, 10.63, 11.56, 12.24, 7.33, 8.54]  # Adjust for your data

# Define the custom RGB colors (same as before)
custom_color_1 = (255/255, 111/255, 97/255)
custom_color_2 = (166/255, 220/255, 239/255)
custom_color_3 = (210.5/255, 165.5/255, 168/255)
...
for i in range(len(bar_positions)):
    if i == 0:
        plt.bar(bar_positions[i], means[i], width=bar_widths[i], color=custom_color_3, 
                edgecolor=custom_color_3, linewidth=lw)
    elif i == 1:
        ...
        plt.bar(bar_positions[i], means[i], width=bar_widths[i], color=custom_color_1, 
                edgecolor=custom_color_1, linewidth=lw)
    elif i == 4:
        plt.bar(bar_positions[i], means[i], width=bar_widths[i], color=custom_color_2, 
                edgecolor=custom_color_2, linewidth=lw)
    elif i == 5:
        plt.bar(bar_positions[i], means[i], width=bar_widths[i], color=custom_color_3, 
                edgecolor=custom_color_3, linewidth=lw)
    elif i == 6:
...
plt.savefig('SupFig20.pdf', format='pdf', dpi=300, bbox_inches='tight')
