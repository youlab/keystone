...
if setDist == 0
    Ag = 1e16; %infinitely large number
else
    Ag = 7e-1; % % lowering this parameter effectively increases the time spent 
% by branch extension towards lower antibiotic concentration vs. higher nutrient concentration
end
...
%             if min(N(:)) > Ag*KN % Condition 02, seemed to enable enhanced colonization when tested for d = 12 mm
     ...
     if mod(i*dt, 12) == 0
       ...
       fig1 = figure(2); % ES
        ...
        drawnow
        saveas(fig1, [pwd '/' num2str(day) '/1Kp1Pa_d' num2str(2*abs(x0)) 'mm_CTX2p5_gradients_t' num2str(i*dt) 'h.png' ])
        ...
        writematrix(C, [pwd '/' num2str(day) '/1Kp1Pa_d' num2str(2*abs(x0)) 'mm_Pa_t' num2str(i*dt) 'h.csv'])
        writematrix(N, [pwd '/' num2str(day) '/1Kp1Pa_d' num2str(2*abs(x0)) 'mm_N_t' num2str(i*dt) 'h.csv' ])
        writematrix(K, [pwd '/' num2str(day) '/1Kp1Pa_d' num2str(2*abs(x0)) 'mm_Kp_t' num2str(i*dt) 'h.csv' ])
        writematrix(A, [pwd '/' num2str(day) '/1Kp1Pa_d' num2str(2*abs(x0)) 'mm_A_t' num2str(i*dt) 'h.csv' ])
        writematrix(B, [pwd '/' num2str(day) '/1Kp1Pa_d' num2str(2*abs(x0)) 'mm_B_t' num2str(i*dt) 'h.csv' ])
...
writematrix(output, [pwd '/' num2str(day) '/1Kp1Pa_d' num2str(2*abs(x0)) 'mm_t_vs_Biomass.csv'])


MaxVec = MaxVec(1:jj-1, :);
MV = MaxVec;

writematrix(MV, [pwd '/' num2str(day) '/1Kp1Pa_d' num2str(2*abs(x0)) 'mm_t_vs_MaxChanges.csv'])
